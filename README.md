...
`docker build -t glossa_psych_cdr .`

If you're using a machine with an ARM processor, such as Apple Silicon, use the following command to build an image:

`docker buildx build --platform linux/amd64 -t glossa_psych_cdr .`

Then, type:

`docker run --rm -p 8787:8787 -e PASSWORD=password glossa_psych_cdr`
